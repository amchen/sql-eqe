if exists(select * from SYSOBJECTS where ID = object_id(N'absp_Util_CreateSysTableScriptWithCollation') and objectproperty(id,N'IsProcedure') = 1)
begin
   drop procedure absp_Util_CreateSysTableScriptWithCollation
end
go

create procedure absp_Util_CreateSysTableScriptWithCollation
	@ret_sqlScript 	varchar(max) output ,
	@baseTableName 	char(120) ,
	@newTableName 	char(120) = '' ,
...
begin
 ...
 print @destDbName
		...
		Set @sSql = @sSql + ' create table ' + dbo.trim(@schemaName) + '.' + Ltrim(Rtrim(@targetTableName)) + ' ('
		...
		Insert Into @PK Exec sp_pkeys @baseTableName2,@table_owner = @schemaName
		...
		Select distinct
			...
			'Length'	= convert(int, c.max_length),
			'Prec'		= case when charindex(type_name(c.system_type_id), @numtypes) > 0
					       then convert(char(5),ColumnProperty(c.object_id, c.name, 'precision'))
					       else '     ' end,
			'Scale'		= case when charindex(type_name(c.system_type_id), @numtypes) > 0
					       then convert(char(5),OdbcScale(c.system_type_id,scale))
					       else '     ' end,
			'Nullable'	= case when c.is_nullable = 0 then @no else @yes end,
			'N', 'N', 'N',Column_Id, 0, 0, 0

		From sys.all_columns c inner join sys.tables t 
		on c.object_id = t.object_id 
		Where object_name(c.object_id)= @baseTableName and SCHEMA_NAME(schema_id)=@schemaName 
		Order By Column_Id
		...
		set @cnstname =''
		select top 1 @cnstname = name,@NonClustered=type_desc  from sys.indexes where object_name(object_id) =@baseTableName and is_primary_key =1
		select @columnlist = COALESCE(@columnlist + ',', '') + '[' + index_col(@baseTableName, indID, KeyNo) + ']'
				from Sys.SysIndexKeys t1,sys.indexes t2 Where t1.Id = t2.object_id
									And t1.indID =t2.index_id  and object_name(t2.object_id) =@baseTableName and  is_primary_key =1	 Order By t1.KeyNo
		...
		IF @makeIndex > 0			-- Create Index Script
			...
			Select @UniqueInd = '', @ClusteredInd = '', @FileGroup = ''
				...
				Declare Cur_Index Cursor LOCAL For Select i.index_id, i.name, i.is_unique, INDEXPROPERTY(i.object_id, i.name,'IsClustered'),d.name as FileGrp
											from sys.indexes i, sys.stats s, sys.data_spaces d,sys.tables t
											Where i.object_id = s.object_id
											And i.index_id = s.stats_id
											And i.object_id =t.object_id 
											And object_name(i.object_id) = @baseTableName
					...
					Fetch Next From Cur_Index Into @index_id, @IndName, @index_is_unique, @index_is_clustered, @FileGroup
						While @@Fetch_Status = 0
							Begin
								...
								Set @IndexScript = @IndexScript + 'CREATE' + @UniqueInd + @ClusteredInd + ' INDEX ' + @IndName + ' ON ' + dbo.trim(@schemaName ) + '.'+ Ltrim(Rtrim(@targetTableName)) + ' ( '
