if exists(select 1 from SYSOBJECTS where ID = OBJECT_ID(N'absp_PopulateParentList') and OBJECTPROPERTY(id,N'IsProcedure') = 1)
begin
   drop procedure absp_PopulateParentList;
end
go

create  procedure absp_PopulateParentList @nodeKey int, @nodeType int
	...
	begin
		...
		--Get the list of all the child nodes--
         ...
         begin
			...
			execute absp_PopulateParentList @childKey, @childType
			begin transaction;
