...
/****** Object:  StoredProcedure [dbo].[absp_Util_UnloadData4Bldzipct]    Script Date: 08/20/2014 11:19:24 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[absp_Util_UnloadData4Bldzipct]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[absp_Util_UnloadData4Bldzipct]
GO
...
CREATE procedure  [dbo].[absp_Util_UnloadData4Bldzipct]
    @unloadType     char(1),
...
====================================================================================================
				kaz renamed for Bldzipct
DB Version:     SQL2008R2
...
Example:        exec absp_Util_UnloadData4Bldzipct
                    @unloadType='t',
                    @unloadText='esdl',
...
begin
    ...
    declare @sSql           varchar(7999)   -- xp_cmdshell only works with varchar(-U8000), not varchar(max)
    ...
    begin
        ...
        set @sSql = 'insert into #TMPCOLUMNS (CNAME, CTYPE, CLENGTH) select NAME, TYPE_NAME(USER_TYPE_ID), convert(int, MAX_LENGTH) ' +
...
-- special handling for Lat/Lon columns
set @unloadText = replace(@unloadText,',Lat,Lon,',',cast(Lat as numeric(12,6)),cast(Lon as numeric(12,6)),');

-- special handling for Lat/Lon columns
set @unloadText = replace(@unloadText,',Dist_Coast,Grnd_Elev,Terr_Feat1,Terr_Feat2,',',cast(Dist_Coast as numeric(12,6)),cast(Grnd_Elev as numeric(12,6)),cast(Terr_Feat1 as numeric(12,6)),cast(Terr_Feat2 as numeric(12,6)),');

-- special handling for Lat/Lon columns
set @unloadText = replace(@unloadText,',Soil_Fact',',cast(Soil_Fact as numeric(12,6))');
		...
		set @sSql = 'bcp "' + @unloadText + '" queryout ' + @outFile + ' -S ' + @hostName + ' -c -C1252 -t "' + @delimiter + '"'
