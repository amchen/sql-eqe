if exists(SELECT * from SYSOBJECTS where ID = object_id(N'absp_InfoTableAttribGetCurrencyMigrationProgress') and OBJECTPROPERTY(id,N'IsProcedure') = 1)
begin
   drop procedure absp_InfoTableAttribGetCurrencyMigrationProgress
end
go
 
create procedure absp_InfoTableAttribGetCurrencyMigrationProgress  @setting bit out, @nodeKey integer 
/*
##BD_BEGIN  
<font size ="3">
...
Purpose:
     ...
     This procedure gets the 'CF_MIGRATION_IN_PROGRESS' Attribute settings for the given currency node and returns it in an
     output parameter. 
