if exists(SELECT * from SYSOBJECTS where ID = object_id(N'absp_InfoTableAttribGetCurrencyCopyInProgress') and OBJECTPROPERTY(id,N'IsProcedure') = 1)
begin
   drop procedure absp_InfoTableAttribGetCurrencyCopyInProgress
end
go
 
create procedure absp_InfoTableAttribGetCurrencyCopyInProgress  @setting bit out, @nodeKey integer 
/*
##BD_BEGIN  
<font size ="3">
...
Purpose:
     ...
     This procedure gets the 'CF_COPY_IN_PROGRESS' Attribute settings for the given currency node and returns it in an
     output parameter. 
