IF OBJECT_ID('dbo.absp_QA_BlitzIndex') IS NOT NULL
	DROP PROCEDURE dbo.absp_QA_BlitzIndex;
GO

CREATE PROCEDURE dbo.absp_QA_BlitzIndex
...
/*
absp_QA_BlitzIndex(TM) v2.02 - Jan 30, 2014
...
How to use:
--	Diagnose:
		EXEC dbo.absp_QA_BlitzIndex @DatabaseName='AdventureWorks';
--	Return detail for a specific table:
		EXEC dbo.absp_QA_BlitzIndex @DatabaseName='AdventureWorks', @SchemaName='Person', @TableName='Person';
...
Known limitations of this version:
 ...
 Thanks for using absp_QA_BlitzIndex(TM)!
 Sincerely,
 The Humans of Brent Ozar Unlimited(TM)

CHANGE LOG (last five versions):
	Jan 30, 2014 (v2.02)
		Standardized calling parameters with sp_AskBrent(TM) and absp_QA_BlitzIndex(TM). (@DatabaseName instead of @database_name, etc)
	...
	May 14, 2013 (v2.0) - Added data types and max length to all columns (keys, includes, secret columns)
		...
		Fixed bug where you couldn't see detailed view for indexed views.
			(Ex: EXEC dbo.absp_QA_BlitzIndex @DatabaseName='AdventureWorks', @SchemaName='Production', @TableName='vProductAndDescription';)
...
BEGIN TRY
	BEGIN
		...
		BEGIN
			SET @msg=N'absp_QA_BlitzIndex is only supported on SQL Server 2005 and higher. The version of this instance is: ' + @SQLServerProductVersion;
			RAISERROR(@msg,16,1);
		END
		...
		RAISERROR(N'Starting run. absp_QA_BlitzIndex(TM) v2.02 - Jan 30, 2014', 0,1) WITH NOWAIT;
		...
		[more_info] AS N'EXEC dbo.absp_QA_BlitzIndex @DatabaseName=' + QUOTENAME([database_name],'''') +
...
BEGIN
	...
	WITH table_mode_cte AS (
		...
		SELECT 	N'Database ' + QUOTENAME(@DatabaseName) + N' as of ' + convert(nvarchar(16),getdate(),121) +
				N' (absp_QA_BlitzIndex(TM) v2.02 - Jan 30, 2014)' ,
