...
if exists(SELECT 1 from SYSOBJECTS where ID = object_id(N'absp_QA_CleanUpSchema') and OBJECTPROPERTY(id,N'IsProcedure') = 1)
begin
   drop procedure absp_QA_CleanUpSchema
end
go
...
*********************************************************/  
CREATE PROCEDURE absp_QA_CleanUpSchema
(
...
If it is helpful, Please send your comments ranjith_842@hotmail.com or visit http://www.ranjithk.com
 ...
 To delete all non-dbo tables
	exec absp_QA_CleanUpSchema 'a', 'a'
	exec absp_QA_CleanUpSchema 'a', 't'	// drop views etc.
...
begin
	print 'Hello absp_QA_CleanUpSchema ' + @cleanupSchemaName + ', ''' + @WorkTest + '''';
	exec absp_QA_CleanUpSchema @cleanupSchemaName, @WorkTest
	FETCH next from curs1 into @cleanupSchemaName
end
close curs1
deallocate curs1
