if exists(select 1 from sysobjects where ID = object_id(N'absp_GenerateInsertDataBySysColumn') and objectproperty(id,N'IsProcedure') = 1)
begin
   drop procedure absp_GenerateInsertDataBySysColumn
end
go

create procedure absp_GenerateInsertDataBySysColumn
...
begin
    ...
    -- init variables
	set @me = 'absp_GenerateInsertDataBySysColumn';
    set @columnList = '';
	...
	begin
		set @msg = 'Error: Table ' + @tableName + '.' + @columnName + ' not found in INFORMATION_SCHEMA.COLUMNS'
        exec absp_MessageEx @msg
        select 'ERROR_' + @tableName + '.' + @columnName + '_NOT_IN_INFORMATION_SCHEMA.COLUMNS'
    end
    else
	begin
        ...
        select ORDINAL_POSITION as FIELDNUM, ltrim(rtrim(COLUMN_NAME)) as FIELDNAME into #tmp_1
			from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = @tableName order by FIELDNUM
		...
		set @sql1 = 'select DATA_TYPE, COLUMN_NAME from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = '''+@tableName +''' order by ORDINAL_POSITION asc'
		...
		--SQL2005 does not required explicit commit
		--insert tmp_3 values ('    commit;', 999)
		if @hasIdentity = 1
		...
		--set @sql = 'unload select INSERT_CLAUSE from tmp_3 order by ROW_NO to ''' + @filename + ''' delimited by ''|'' format ascii quotes off';
