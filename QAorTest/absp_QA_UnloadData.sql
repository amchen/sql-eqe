if exists(select 1 from SYSOBJECTS where id = object_id(N'absp_QA_UnloadData') and objectproperty(ID,N'IsProcedure') = 1)
begin
    drop procedure absp_QA_UnloadData
end
go

create procedure  absp_QA_UnloadData
    @unloadType     char(1),
...
Example:        exec absp_QA_UnloadData
                    @unloadType='t',
                    @unloadText='esdl',
...
begin
    ...
    begin
        ...
        -- Add the database name for multiple databases
        set @sSql = @sSql + ' -d "' + db_name() + '"'
