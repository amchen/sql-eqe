if exists(select * from SYSOBJECTS where ID = object_id(N'absp_InvalidateResultsUpDownAndSelf') and objectproperty(ID,N'IsProcedure') = 1)
begin
   drop procedure absp_InvalidateResultsUpDownAndSelf
end
go

create  procedure absp_InvalidateResultsUpDownAndSelf  @nodeKey int, @nodeType int, @isForceInvalidation int=1, @actionType varchar(30) = 'InvalidateUpDownNodesAndSelf'
/*
##BD_BEGIN
<font size ="3">
...
begin
	...
	-- Add a task progress message
	set @taskProgressMsg = 'Getting the list of nodes to be invalidated';
	exec absp_Util_AddTaskProgress @taskKey, @taskProgressMsg, @procID;
	...
	begin
		...
		begin
			...
			-- Add a task progress message
			set @taskProgressMsg = 'Invalidating by EBERunID';
			exec absp_Util_AddTaskProgress @taskKey, @taskProgressMsg, @procID;
