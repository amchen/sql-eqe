if exists(SELECT 1 from SYSOBJECTS where ID = object_id(N'absp_Migr_GetExposureKeysForCalcQFUpdate') and OBJECTPROPERTY(id,N'IsProcedure') = 1)
begin
   drop procedure absp_Migr_GetExposureKeysForCalcQFUpdate
end
go
 
create procedure absp_Migr_GetExposureKeysForCalcQFUpdate  @nodeType integer, @nodeKey integer, @exposureKey integer = 0  
/*
##BD_BEGIN  
<font size ="3">
...
Purpose:
     ...
     This procedure will return all exposure keys under the given portfolio that are marked for CalcQF update.
...
begin
	...
	Select @attrib = AttribVal from AttrDef Where AttribName = @attribName 
	
					
	if @nodeType = 1
	begin
	
		set @sql = 'select ExposureKey from ExposureInfo where Attrib & ' + str(@attrib) + ' = ' + str(@attrib) + ' and ' +
