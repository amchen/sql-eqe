if exists (select 1 from dbo.SYSOBJECTS where ID = object_id(N'dbo.On_insert_of_FLDRMAP_insert_CURRMAP') and objectproperty(id,N'IsTrigger') = 1)
begin
    drop trigger On_insert_of_FLDRMAP_insert_CURRMAP
end
go

create trigger On_insert_of_FLDRMAP_insert_CURRMAP on FLDRMAP
after insert as
/*
##BD_BEGIN
<font size ="3">
...
====================================================================================================
DB Version:     MSSQL
Purpose:        This trigger gets fired on the insertion of a record in the FLDRMAP table and it
...
*/
    ...
    execute absp_messageEx 'TRIGGER On_insert_of_FLDRMAP_insert_CURRMAP starts'

    select @childKey = CHILD_KEY, @childType = CHILD_TYPE from INSERTED

    -- Only continue if the new FLDRMAP record was for either Primary (2) or Reinsurance portfolios (3)
