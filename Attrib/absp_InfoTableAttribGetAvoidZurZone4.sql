if exists(SELECT * from SYSOBJECTS where ID = object_id(N'absp_InfoTableAttribGetAvoidZurZone4') and OBJECTPROPERTY(id,N'IsProcedure') = 1)
begin
   drop procedure absp_InfoTableAttribGetAvoidZurZone4
end
go
 
create procedure absp_InfoTableAttribGetAvoidZurZone4  @setting bit out, @nodeKey integer 
/*
##BD_BEGIN  
<font size ="3">
...
Purpose:
     ...
     This procedure gets the 'AvoidZurZone4' Attribute settings for the given currency node and returns it in an
     output parameter. 
...
begin
	...
	insert into @TableVar exec absp_InfoTableAttribAllGet  64, @nodeKey  
		
	select @setting = SETTING  from @TableVar where ATTRIBUTE = @attribName
	
	select @setting as IsZursZone
end
