if exists(SELECT * from SYSOBJECTS where ID = object_id(N'absp_InfoTableAttribGetCurrencyMigrationNeeded') and OBJECTPROPERTY(id,N'IsProcedure') = 1)
begin
   drop procedure absp_InfoTableAttribGetCurrencyMigrationNeeded
end
go
 
create procedure absp_InfoTableAttribGetCurrencyMigrationNeeded  @setting bit out, @nodeKey integer 
/*
##BD_BEGIN  
<font size ="3">
...
Purpose:
     ...
     This procedure gets the 'CF_MIGRATION_NEEDED' Attribute settings for the given currency node and returns it in an
     output parameter. 
