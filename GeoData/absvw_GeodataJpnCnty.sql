if exists(select * from SYSOBJECTS where ID = object_id(N'absvw_GeodataJpnCnty') and objectproperty(id,N'IsView') = 1)
begin
   drop view absvw_GeodataJpnCnty
end
go
create view absvw_GeodataJpnCnty as
	select 
		1 as GEOROW_KEY, 
		...
		'' as CrestaVintage  
	from 
		CNTYCT02 Z, RRGNLIST RL, COUNTRY C 
	where 
			C.COUNTRYKEY = 3 
