if exists(select * from SYSOBJECTS where ID = object_id(N'absvw_GeodataUsaPrLocator') and objectproperty(id,N'IsView') = 1)
begin
   drop view absvw_GeodataUsaPrLocator
end
go
create view absvw_GeodataUsaPrLocator as
	select 
		1 as GEOROW_KEY, 
...
when W.MAPI_STAT = 8 and SUBSTRING(W.LOCATOR, 5, 1) = '0' then 
 ...
 when W.MAPI_STAT = 7 or W.MAPI_STAT = 6 then
  ...
  as CODEVLOCAL,
	...
	from 
		CNTYCT Z, RRGNLIST RL, COUNTRY C, WCCCODES w 
	where 
			C.COUNTRYKEY = 1 
		and C.COUNTRY_ID = RL.COUNTRY_ID 
		and left(z.FIPS, 2) = left(RL.FIPS,2)
		and (z.state = w.STATE_2 and z.fips=w.fips and Z.STATE = 'PR') 
		and (W.COUNTRY_ID='00' or z.COUNTY=rtrim(ltrim(SUBSTRING(w.LOCATOR,5,10))))
