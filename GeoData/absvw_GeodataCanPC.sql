if exists(select * from SYSOBJECTS where ID = object_id(N'absvw_GeodataCanPC') and objectproperty(id,N'IsView') = 1)
begin
   drop view absvw_GeodataCanPC
end
go
create view absvw_GeodataCanPC as
	select 
		1 as GEOROW_KEY,			
		COUNTRYKEY as COUNTRYKEY, 	ABBREV as COUNTRY_ID,		
		ZIP_CODE as CODE_VALUE, 	ZIP_CODE as CODEVLOCAL,		
		'' as LOCATOR,				CITY as MUN_CODE,				
		cast(206 as smallint) as GEO_STAT, 
