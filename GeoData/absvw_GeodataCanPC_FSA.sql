if exists(select * from SYSOBJECTS where ID = object_id(N'absvw_GeodataCanPC_FSA') and objectproperty(id,N'IsView') = 1)
begin
   drop view absvw_GeodataCanPC_FSA
end
go
create view absvw_GeodataCanPC_FSA as
	select 
		1 as GEOROW_KEY,			
		COUNTRYKEY as COUNTRYKEY, 	ABBREV as COUNTRY_ID,		
		ZIP_CODE as CODE_VALUE, 	ZIP_CODE as CODEVLOCAL,		
		'' as LOCATOR,				CITY as MUN_CODE,				
		cast(203 as smallint) as GEO_STAT, 
		...
		'' as CrestaVintage  
	from 
		ZIPCT01 Z, RRGNLIST RL, COUNTRY C 
	where 
			C. COUNTRYKEY = 2 
		and C.COUNTRY_ID = RL.COUNTRY_ID 
		and Z.FIPS =RL.FIPS 
		and LEN(Z.ZIP_CODE) = 3
