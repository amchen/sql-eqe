if exists(select * from SYSOBJECTS where ID = object_id(N'absvw_GeodataJpnPC') and objectproperty(id,N'IsView') = 1)
begin
   drop view absvw_GeodataJpnPC
end
go
create view absvw_GeodataJpnPC as
	select	
		1 as GEOROW_KEY, 
		...
		'' as CrestaVintage  
	from 
		ZIPCT02 Z, RRGNLIST RL, COUNTRY C 
	where 
			C.COUNTRYKEY = 3 
