if exists(select * from SYSOBJECTS where ID = object_id(N'absp_GetPolicyBrowserInfo') and objectproperty(ID,N'IsProcedure') = 1)
begin
   drop procedure absp_GetPolicyBrowserInfo
end
go

create  procedure absp_GetPolicyBrowserInfo  @nodeKey int, 
					     @nodeType int, 
					     @exposureKey int =-1, 
					     @accountKey int=-1,
					     @sessionID int,
...
=================================================================================
DB Version:    MSSQL
Purpose: The procedure will get the summary information from the PolicyBrowserInfo table 
...
begin
	BEGIN TRY
		...
		exec absp_MessageEx  'Begin absp_GetPolicyBrowserInfo'
		
		set @debug = 0;
		set @tRows= 20000;
		set @tmpTbl='TMP_POL_SUMMARY' + dbo.trim(cast(@sessionId as varchar(50)))
		set @tmpCntTbl='TMP_POL_SUMMARY_CNT' + dbo.trim(cast(@sessionId as varchar(50)))
		...
		if not exists(select 1 from sys.tables where name=@tmpTbl)
	 	begin
			--execute absp_Util_CreateTableScript @sql output,'PolicyBrowserInfo', @tmpTbl,'',0,0,0
			set @sql='select * into ' + @tmpTbl + ' from PolicyBrowserInfo where 1=2'
			exec (@sql)
			...
			begin
				...
				begin
					...
					set @sql = 'insert into ' + @tmpTbl + 
						' (ExposureKey,AccountKey,PolicyKey,AccountNumber, PolicyNumber, PolicyName,InceptionDate, ExpirationDate, PolicyStatusDisplayName,LineOfBusinessDisplayName,ProRata, 
						Currency,UnderCover,PerilDisplayName,CoverageDisplayName,ConditionTypeDisplayName,StepTemplateDisplayName,Limit,Deductible,
						MinDeductible,MaxDeductible ,Priority,ConditionName,LimitAssured , DeductibleAssured ,FinancialModelType,IsValid)
					    select top (' + dbo.trim(cast(@tRows as varchar)) + ') T1.ExposureKey,AccountKey,PolicyKey,AccountNumber, PolicyNumber, PolicyName,InceptionDate, ExpirationDate, PolicyStatusDisplayName,
						LineOfBusinessDisplayName,ProRata,Currency,UnderCover,PerilDisplayName,CoverageDisplayName,ConditionTypeDisplayName,StepTemplateDisplayName,
						Limit ,Deductible,MinDeductible,MaxDeductible,Priority,ConditionName, LimitAssured , DeductibleAssured ,FinancialModelType,IsValid
						from PolicyBrowserInfo T1 inner join  #TMP_EXP T2  on T1.ExposureKey=T2.ExposureKey' ;

					set @RowCntSql='select count(*) as CNT into ' + @tmpCntTbl +' from PolicyBrowserInfo T1 inner join #TMP_EXP T2  on T1.ExposureKey=T2.ExposureKey';
				...
				begin	
					...
					set @sql = 'insert into ' + @tmpTbl + 
						...
						Currency,UnderCover,PerilDisplayName,CoverageDisplayName,ConditionTypeDisplayName, StepTemplateDisplayName,Limit ,Deductible,
						MinDeductible,MaxDeductible ,Priority,ConditionName,LimitAssured,DeductibleAssured,FinancialModelType,IsValid)
					    select top (' + dbo.trim(cast(@tRows as varchar)) + ') ExposureKey,AccountKey,PolicyKey,AccountNumber, PolicyNumber, PolicyName,InceptionDate, ExpirationDate, PolicyStatusDisplayName,
						LineOfBusinessDisplayName, ProRata, Currency, UnderCover,PerilDisplayName,CoverageDisplayName,ConditionTypeDisplayName,StepTemplateDisplayName,
						Limit ,Deductible,MinDeductible,MaxDeductible,Priority,ConditionName, LimitAssured , DeductibleAssured ,' +  cast(@financialModelType as varchar) + ',IsValid
						from PolicyBrowserInfo ' +
						' where ExposureKey = ' + cast(@exposureKey as varchar) + 
						' and AccountKey= ' + cast(@nodeKey	as varchar) 

					set @RowCntSql='select count(*) as CNT into ' + @tmpCntTbl + ' from PolicyBrowserInfo where ExposureKey = ' + cast(@exposureKey as varchar) + ' and AccountKey =' + cast(@nodeKey as varchar);
		...
		set @sql ='select ReportsAvailable,ExposureKey,AccountKey,PolicyKey,AccountNumber, PolicyNumber, PolicyName,InceptionDate, ExpirationDate, PolicyStatusDisplayName, 
			LineOfBusinessDisplayName , ProRata, Currency, 
			UnderCover,PerilDisplayName,CoverageDisplayName,ConditionTypeDisplayName, StepTemplateDisplayName,
			case when Limit>0 and Limit<=1 then dbo.trim( str(Limit * 100  ,50,3) + ''%'')
				else cast(cast(Limit as decimal) as varchar(50))
			end as Limit,
			case when cast(Deductible as float) =-1 then  '' 1 day''
			...
			case when cast(MinDeductible as float) =-1 then  '' 1 day''
			...
			Priority,ConditionName,
			LimitAssured,DeductibleAssured,FinancialModelType,IsValid
		from ' + @tmpTbl + ' where PolicyBrowserInfoRowNum between ' + cast(@startRowNum as varchar) + ' and ' + cast(@endRowNum as varchar) 
		+ ' order by PolicyBrowserInfoRowNum ' +
		+ ' OPTION(RECOMPILE)'
		exec(@sql)
	...
	END CATCH
	exec absp_MessageEx  'End absp_GetPolicyBrowserInfo'
end