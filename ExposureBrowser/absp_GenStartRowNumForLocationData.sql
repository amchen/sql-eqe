if exists(select 1 from SYSOBJECTS where ID = object_id(N'absp_GenStartRowNumForLocationData') and objectproperty(ID,N'IsProcedure') = 1)
begin
   drop procedure absp_GenStartRowNumForLocationData
end
go

CREATE procedure [dbo].[absp_GenStartRowNumForLocationData] @nodeKey int=-1, @nodeType int=-1,@chunkSize int
...
begin
	...
	begin	
			begin
				insert into #TMP_STARTROWNUM exec  absp_GenStartRowNumForLocation  -1,@nodeType, @expKey,@chunkSize
			end
