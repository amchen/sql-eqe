if exists(select 1 from SYSOBJECTS where ID = object_id(N'absp_GenStartRowNumForLocationCondition') and objectproperty(ID,N'IsProcedure') = 1)
begin
   drop procedure absp_GenStartRowNumForLocationCondition
end
go

create procedure absp_GenStartRowNumForLocationCondition @nodeKey int=-1, @nodeType int=-1, @exposureKey int=-1,@chunkSize int
...
begin
	...
	begin	
		...
		--select  @startRowNum= isnull(min(SiteRowNum),-1) from Site where ExposureKey = @expKey option (recompile);
		...
		--begin
			...
			begin
				...
				select top(1) @startRowNum=SiteRowNum from   Site 
					where exposureKey=@expKey and SiteRowNum > @startRowNum order by SiteRowNum option (recompile);
