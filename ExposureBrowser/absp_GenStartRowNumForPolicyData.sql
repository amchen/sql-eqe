if exists(select 1 from SYSOBJECTS where ID = object_id(N'absp_GenStartRowNumForPolicyData') and objectproperty(ID,N'IsProcedure') = 1)
begin
   drop procedure absp_GenStartRowNumForPolicyData
end
go

CREATE procedure [dbo].[absp_GenStartRowNumForPolicyData] @nodeKey int=-1, @nodeType int=-1,@chunkSize int
...
begin
	...
	begin	
			begin
				insert into #TMP_STARTROWNUM exec  absp_GenStartRowNumForPolicy  -1,@nodeType, @expKey,@chunkSize
			end
