if exists(select 1 from SYSOBJECTS where ID = object_id(N'absp_GetSiteConditionBrowserData') and objectproperty(id,N'IsProcedure') = 1)
begin
   drop procedure absp_GetSiteConditionBrowserData
end
 go

create procedure absp_GetSiteConditionBrowserData @nodeKey int, @nodeType int,@financialModelType int, @pageNum int,@pageSize int=1000,@userKey int=1,@debug int=0					
as
begin
	set nocount on
	...
	begin
		select @rowCnt=TotalCount from FilteredStatReport where Category='Site Conditions' and nodeKey=@nodeKey and NodeType=@nodeType
			...
			set identity_insert #TmpSiteCon off
			 ...
			 select distinct
				...
				set @sql = 'select distinct T1.*,AccountNumber,SiteNumber,StructureNumber,SiteName,StructureName,T2.CurrencyCode as CurrencyCode,
					isnull(ErrorMessage,' + @errorStr+ ') as ErrorMessage,' +dbo.trim(cast(@pageNum as varchar(30))) + ' as PageNumber,RowNum  from #TmpSiteCon T1 ' +
		...
		begin	
			set @sql = 'select distinct T1.*,AccountNumber,SiteNumber,StructureNumber,SiteName,StructureName,T2.CurrencyCode as CurrencyCode,'''' as ErrorMessage,' +dbo.trim(cast(@pageNum as varchar(30))) + ' as PageNumber,RowNum  from SiteCondition T1 inner join ' + @tableName + ' T2 
				on T1.SiteConditionRowNum=T2.SiteConditionRowNum ' +
				'  where  RowNum>=' + dbo.trim(cast(@startRowNum as varchar(30))) + ' and RowNum<' + dbo.trim(cast(@endRowNum as varchar(30))) + 
				' and FinancialModelType=' + dbo.trim(cast(@financialModelType as varchar(30))) ;
			set @sql = @sql + ' order by RowNum'
