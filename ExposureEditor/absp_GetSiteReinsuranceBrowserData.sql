if exists(select 1 from SYSOBJECTS where ID = object_id(N'absp_GetSiteReinsuranceBrowserData') and objectproperty(id,N'IsProcedure') = 1)
begin
   drop procedure absp_GetSiteReinsuranceBrowserData
end
 go

create procedure absp_GetSiteReinsuranceBrowserData @nodeKey int, @nodeType int, @financialModelType int, @pageNum int,@pageSize int=1000,@userKey int=1,@debug int=0					
as
begin
	set nocount on
	...
	if @InProgress=1
	begin
		select T1.*,'' as AccountNumber,'' as SiteNumber,'' as ReinsurerName,'' as TreatyTagName,'' as ErrorMessage,0 as PageNumber,0 as RowNum from Reinsurance T1 where 1=0; 
		return;
	end	
	...
	begin
		select @rowCnt=TotalCount from FilteredStatReport where Category='Site Reinsurance' and nodeKey=@nodeKey and NodeType=@nodeType
		...
		begin
			 ...
			 set @sql = 'insert into #TmpRein  (' + @fieldNames + ')  select T1.*	from Reinsurance T1 inner join ' + @tableName + ' T2 
				...
				' and FinancialModelType=' + dbo.trim(cast(@financialModelType as varchar(30))) + ' and AppliesTo=''S''';
			...
			--Get the final query-- 
			set @sql = 'select distinct T1.*,AccountNumber,SiteNumber,R.Name  as ReinsurerName,T.Name  as TreatyTagName,isnull(ErrorMessage,' + @errorStr + ') as ErrorMessage,' +
				dbo.trim(cast(@pageNum as varchar(30))) + ' as PageNumber,RowNum  from #TmpRein T1 ' +
		...
		begin		
			set @sql = 'select distinct T1.*,AccountNumber,SiteNumber,R.Name  as ReinsurerName,T.Name  as TreatyTagName,'''' as ErrorMessage,' +
				dbo.trim(cast(@pageNum as varchar(30))) + ' as PageNumber,RowNum  from Reinsurance T1 inner join ' + @tableName + ' T2 
				on T1.ReinsuranceRowNum=T2.ReinsuranceRowNum ' +
				' inner join Reinsurer R on T1.ReinsurerID=R.ReinsurerID '+
				' inner join TreatyTag T on T1.TreatyTagID=T.TreatyTagID '+
				'  where RowNum>=' + dbo.trim(cast(@startRowNum as varchar(30))) + ' and RowNum<' + dbo.trim(cast(@endRowNum as varchar(30))) + 
