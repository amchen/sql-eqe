if exists(select 1 from SYSOBJECTS where ID = object_id(N'absp_GetAccountReinsuranceBrowserData') and objectproperty(id,N'IsProcedure') = 1)
begin
   drop procedure absp_GetAccountReinsuranceBrowserData
end
 go

create procedure absp_GetAccountReinsuranceBrowserData @nodeKey int, @nodeType int,@financialModelType int, @pageNum int,@pageSize int=1000,@userKey int=1,@debug int=0					
as
begin
	set nocount on
	...
	if @InProgress=1 
	begin
		select T1.*,'' as AccountNumber,'' as ReinsurerName,'' as TreatyTagName,'' as ErrorMessage,0 as PageNumber,0 as RowNum from Reinsurance T1 where 1=0; 
		return;
	end	
	...
	select @rowCnt=TotalCount from FilteredStatReport where category='AccountReinsurance' and nodeKey=@nodeKey and NodeType=@nodeType
	...
	begin
		...
		begin
			 ...
			 set @sql = 'insert into #TmpRein (' + @fieldNames + ')  select T1.*	from Reinsurance T1 inner join ' + @tableName + ' T2 
				...
				' and FinancialModelType=' + dbo.trim(cast(@financialModelType as varchar(30))) + ' and AppliesTo=''A''';
			...
			set @sql = 'select distinct T1.*,AccountNumber,R.Name  as ReinsurerName,T.Name  as TreatyTagName,
			isnull(ErrorMessage,' + @errorStr + ') as ErrorMessage,' +dbo.trim(cast(@pageNum as varchar(30))) + ' as PageNumber,RowNum  
			from #TmpRein T1 inner join ' + @tableName + ' T2 on T1.ReinsuranceRowNum=T2.ReinsuranceRowNum '+
		...
		begin	
			set @sql = 'select distinct T1.*,AccountNumber,R.Name  as ReinsurerName,T.Name  as TreatyTagName,'''' as ErrorMessage,' +dbo.trim(cast(@pageNum as varchar(30))) + ' as PageNumber,RowNum  from Reinsurance T1 inner join ' + 
				@tableName + ' T2 on T1.ReinsuranceRowNum=T2.ReinsuranceRowNum ' +
				' inner join Reinsurer R on T1.ReinsurerID=R.ReinsurerID '+
				' inner join TreatyTag T on T1.TreatyTagID=T.TreatyTagID '+
				'  where RowNum>=' + dbo.trim(cast(@startRowNum as varchar(30))) + ' and RowNum<' + dbo.trim(cast(@endRowNum as varchar(30))) + 
				' and FinancialModelType=' + dbo.trim(cast(@financialModelType as varchar(30))) + ' and AppliesTo=''A''';
