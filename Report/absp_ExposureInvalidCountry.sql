if exists(select 1 from dbo.SYSOBJECTS where ID = object_id(N'dbo.absp_ExposureInvalidCountry') and objectproperty(id,N'IsProcedure') = 1)
begin
	drop procedure absp_ExposureInvalidCountry;
end
go

create procedure absp_ExposureInvalidCountry @nodeType int, @nodeKey int
as
BEGIN TRY
	...
	BEGIN
		...
		SELECT coalesce(t.Country, e.CountryCode) as Country, e.CurrencyCode, e.CBPerilID, e.ExposureKey, e.StructureKey, e.NumStructures,
		...
		INTO #x1
		FROM #xExposureImportCurrencyByCountry
		GROUP BY CBPerilID
		,Country
		,CurrencyCode
		,ExposureKey
		...
		--final rollup
		insert ExposureSummaryReport (NodeKey,NodeType,EngineCallID,IsRegion,Peril,Country,CurrencyCode,NumStructures,Validated,NotValidated,Total)
...
END CATCH
go
--absp_ExposureInvalidCountry @nodeType=7, @nodeKey=1
