if exists(select 1 from dbo.SYSOBJECTS where ID = object_id(N'dbo.absp_ExposureStructureTypeByRegion') and objectproperty(id,N'IsProcedure') = 1)
begin
	drop procedure absp_ExposureStructureTypeByRegion;
end
go

create procedure absp_ExposureStructureTypeByRegion @nodeType int, @nodeKey int
as
BEGIN TRY
	...
	select RRgn_Key,Name into #Regions from RRegions
	CREATE NONCLUSTERED INDEX #Regions_i1 ON #Regions  (RRgn_Key);
	...
	BEGIN
		...
		SELECT t.country, r.Name as 'Region', e.ExposureKey, e.CoverageID, e.Country_ID, e.CBPerilID, e.PerilType, e.StructureID, e.StructureKey, e.NumStructures, e.NumBuildings, e.Value,
		...
		FROM ExposureReport e WITH (NOLOCK)
			...
			INNER JOIN #Regions r ON e.RegionKey = r.RRgn_Key
		...
		SELECT Country, Region, Country_ID, CBPerilID, StructureID, PerilType, ExposureKey, StructureKey, NumStructures, NumBuildings,
		sum(BValue)  as BV,
		sum(CValue)  as CV,
		sum(BIValue) as BI
		INTO #x1
		FROM(SELECT Country, Region, e.Country_ID, e.CBPerilID, e.StructureID, e.PerilType, e.ExposureKey, e.StructureKey, e.NumStructures, e.NumBuildings, BValue, CValue, BIValue
			FROM #xExposureStructureTypeByCountry AS e) as #tmp
		GROUP BY Country,Region,Country_ID,CBPerilID,StructureID,PerilType,ExposureKey,StructureKey,NumStructures,NumBuildings;

		CREATE NONCLUSTERED INDEX #x2_i1 ON #x1  (Country_ID,CBPerilID,StructureID,PerilType,StructureKey);

		--Final rollup
		insert ExposureSummaryReport (NodeKey,NodeType,EngineCallID,IsRegion,Country,Region,Peril,StructureType,NumStructures,NumBuildings,BuildingValue,ContentsValue,TotalPropertyValue,BIValue,Total)
			SELECT @NodeKey,@NodeType,51,1
			,Country, Region
			...
			GROUP BY Country,Region,p.PerilDisplayName,Comp_Descr;

		if (@@rowcount = 0) begin
			insert ExposureSummaryReport (NodeKey,NodeType,EngineCallID,IsRegion,Country,Region,Peril,StructureType,NumStructures,NumBuildings,BuildingValue,ContentsValue,TotalPropertyValue,BIValue,Total)
				values (@NodeKey,@NodeType,51,1,NULL,NULL,'No records to display',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
		end
	END
...
END CATCH
go
--absp_ExposureStructureTypeByRegion @nodeType=7, @nodeKey=1
