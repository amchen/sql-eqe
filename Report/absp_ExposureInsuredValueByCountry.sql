if exists(select 1 from dbo.SYSOBJECTS where ID = object_id(N'dbo.absp_ExposureInsuredValueByCountry') and objectproperty(id,N'IsProcedure') = 1)
begin
	drop procedure absp_ExposureInsuredValueByCountry
end
go

create procedure absp_ExposureInsuredValueByCountry @nodeType int, @nodeKey int
as
BEGIN TRY
	...
	BEGIN
		...
		SELECT t.country, e.ExposureKey,
			e.CBPerilID, e.StructureKey, e.NumStructures, e.NumBuildings, e.Value,
		...
		INTO #xExposureInsuredValueByCountry
		...
		CREATE NONCLUSTERED INDEX #x1_i1 ON #xExposureInsuredValueByCountry (ExposureKey,StructureKey,CBPerilID);

		--summing values particularly by exposurekey and structurekey (to prevent double counting); also creating index
		SELECT Country, CBPerilID, ExposureKey, StructureKey, NumStructures, NumBuildings,
		sum(BValue)  as BV,
		sum(CValue)  as CV,
		sum(BIValue) as BI
		INTO #x1
		FROM(SELECT Country, e.CBPerilID, e.ExposureKey, e.StructureKey, e.NumStructures, e.NumBuildings, BValue, CValue, BIValue
			FROM #xExposureInsuredValueByCountry AS e) as #tmp
		GROUP BY Country,CBPerilID,ExposureKey,StructureKey,NumStructures,NumBuildings;

		CREATE NONCLUSTERED INDEX #x2_i1 ON #x1 (CBPerilID);

		--Final rollup
		insert ExposureSummaryReport (NodeKey,NodeType,EngineCallID,IsRegion,Country,Peril,NumStructures,NumBuildings,BuildingValue,ContentsValue,TotalPropertyValue,BIValue,Total)
			SELECT @NodeKey,@NodeType,50,0
			,Country
			...
			GROUP BY Country,p.PerilDisplayName;

		if (@@rowcount = 0) begin
			insert ExposureSummaryReport (NodeKey,NodeType,EngineCallID,IsRegion,Country,Peril,NumStructures,NumBuildings,BuildingValue,ContentsValue,TotalPropertyValue,BIValue,Total)
				values (@NodeKey,@NodeType,50,0,NULL,'No records to display',NULL,NULL,NULL,NULL,NULL,NULL,NULL);
		end
	END
...
END CATCH
go
--absp_ExposureInsuredValueByCountry @nodeType=7, @nodeKey=1
