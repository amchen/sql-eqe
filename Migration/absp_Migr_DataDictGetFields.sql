if exists(select 1 from SYSOBJECTS where ID = object_id(N'absp_Migr_DataDictGetFields') and objectproperty(id,N'IsProcedure') = 1)
begin
   drop procedure absp_Migr_DataDictGetFields
end
go
create procedure dbo.absp_Migr_DataDictGetFields
...
begin
  ...
  */
	...
	set @fldName=''
	set @ret_fieldNames = ' '
	set @sSql='select  ''' + @correlation +'''+ FIELDNAME from Ver3_16_DICTCOL where  TABLENAME = '''+ rtrim(ltrim(@myTableName))+''' and FIELDNUM<>' + str(@skipKeyFieldNum) + ' order by FIELDNUM asc'
