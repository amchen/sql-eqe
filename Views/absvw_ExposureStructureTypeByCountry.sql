...
if exists (select 1 from sysobjects where id = object_id('absvw_ExposureStructureTypeByCountry') and type = 'V')
   drop view absvw_ExposureStructureTypeByCountry;
go

create view absvw_ExposureStructureTypeByCountry as
select
 ParentKey
,ParentType
,Country
...
,sum(BIValue)       as BI
from
(SELECT     ExposureReport.ParentKey, ExposureReport.ParentType, Country.Country, PTL.PerilDisplayName as Peril, StructureType.Comp_Descr,
...
select StructureType,Country,Peril,NumStructures,BuildingValue,ContentsValue,TotalPropertyValue,BIValue,Total                                     from absvw_ExposureStructureTypeByCountry        where ParentKey=@NodeKey and ParentType=@NodeType
*/
